...
function _split!(
    ...
    if feature_sampler !== nothing
        ...
        @inbounds for i in 1:n_features
            if sampled_mask[features[i]]
                non_consts_used += 1
                features[non_consts_used], features[i] = features[i], features[non_consts_used]
            end
        end
    else
    ...
    @inbounds while ((feature_sampler === nothing && unsplittable) || indf <= non_consts_used) && indf <= n_features
